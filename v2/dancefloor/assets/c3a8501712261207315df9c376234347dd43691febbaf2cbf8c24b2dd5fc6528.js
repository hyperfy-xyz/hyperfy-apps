...
const resetObject = (obj) => {
    ...
    obj.node.scale.set(0.1, 0.1, 0.1)
    // Reset position using same logic as createObject
    obj.node.position.x = obj.x * SPACING
    obj.node.position.z = obj.z * SPACING
}
...
// Modify the control structure to add 'KeyC' handler
const control = app.control({
    onPress: (code) => {
        ...
        if (code === 'KeyK') {
            intensity = Math.max(0.0, intensity - 0.1)
            return true
        }
        // Add wave speed controls
        if (code === 'KeyU') {
            waveSpeed = Math.min(WAVE_SPEED_MAX, waveSpeed + 0.1)
            return true
        }
        if (code === 'KeyJ') {
            waveSpeed = Math.max(WAVE_SPEED_MIN, waveSpeed - 0.1)
            return true
        }
        // Add wave frequency controls
        if (code === 'KeyY') {
            waveFrequency = Math.min(WAVE_FREQ_MAX, waveFrequency + 0.1)
            return true
        }
        if (code === 'KeyH') {
            waveFrequency = Math.max(WAVE_FREQ_MIN, waveFrequency - 0.1)
            return true
        }
        if (code === 'KeyC') {
            isSpikingIntensity = true
            spikeTimer = 0
...
// Add after BASE_HEIGHT constant
const FACE_ROTATIONS = {
    TOP: [0, 0, 0],                    // No rotation needed for top face
    FRONT: [Math.PI/2, 0, 0],          // Rotate front face up
    RIGHT: [0, 0, -Math.PI/2],         // Rotate right face up
    BACK: [-Math.PI/2, 0, 0],          // Rotate back face up
    LEFT: [0, 0, Math.PI/2],           // Rotate left face up
    BOTTOM: [Math.PI, 0, 0]            // Rotate bottom face up
}

// Add new patterns after existing patterns array
const colorPatterns = [
    // Pattern 26: Checkerboard color pattern
    (obj, time) => {
        const isEven = (obj.x + obj.z) % 2 === 0
        const rotation = isEven ? FACE_ROTATIONS.TOP : FACE_ROTATIONS.FRONT
        obj.node.rotation.set(...rotation)
        return 0 // No height change
    },
    // Pattern 27: Concentric rings of color
    (obj, time) => {
        const distance = Math.sqrt(obj.x * obj.x + obj.z * obj.z)
        const ring = Math.floor(distance) % 6
        const rotations = [
            FACE_ROTATIONS.FRONT,
            FACE_ROTATIONS.RIGHT,
            FACE_ROTATIONS.BACK,
            FACE_ROTATIONS.LEFT,
            FACE_ROTATIONS.TOP,
            FACE_ROTATIONS.BOTTOM
        ]
        obj.node.rotation.set(...rotations[ring])
        return 0
    },
    // Pattern 28: Moving color wave
    (obj, time) => {
        const wave = Math.floor((obj.x + time * WAVE_FREQUENCY) % 6)
        const rotation = Object.values(FACE_ROTATIONS)[wave] || FACE_ROTATIONS.TOP
        obj.node.rotation.set(...rotation)
        return Math.sin(time * WAVE_FREQUENCY + obj.x * 0.3) * 0.5
    }
]

// Merge color patterns into main patterns array
