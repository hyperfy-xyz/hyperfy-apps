/**
 * Rain
 * Drag and Drop some Rain into your Hyperfy V2 world.
 * 
...
// Configure UI inputs for the rain particle system
app.configure(() => {
	return [
		{
			type: 'switch',
...
// Get the required meshes from the GLB file
const raindrop = app.get('Icosphere');
if (!raindrop) {
	console.error('Could not find raindrop - make sure your GLB has a mesh named "Icosphere"');
	return;
}
...
if (!ring) {
	console.error('Could not find ring - make sure your GLB has a mesh named "ring"');
	return;
}

// Get the grab mesh (optional, for visibility control)
const grab = app.get('grab');
if (!grab) {
	console.error('Could not find grab - make sure your GLB has a mesh named "grab"');
	return;
}

// Hide the original meshes to avoid duplication
raindrop.visible = false;
ring.visible = false;

// Track splash effects for cleanup
const splashEffects = [];
...
// Apply configuration values from UI inputs
...
// Initialize rain system on client side only
if (world.isClient) {
	console.log('[RAIN] Initializing client-side particle system');
	
	// Particle pool tracking for efficient memory usage
	const rainParticles = [];
	...
	// Initialize rain particle pool for performance optimization
	try {
	...
	// Get an available particle from the pool (reuse system for performance)
	...
	// Spawn a new rain particle with random properties
	function spawnRainParticle() {
		try {
			...
			// Random scale for visual variation
	...
	// Main update loop for rain particles and effects
	app.on('update', (delta) => {
		// Continuous particle emission based on spawn rate
		timeSinceLastBurst += delta;
		if (timeSinceLastBurst >= CONFIG.BURST_INTERVAL) {
			timeSinceLastBurst = 0;
			// Spawn multiple rain particles per burst
			for (let i = 0; i < CONFIG.PARTICLES_PER_BURST; i++) {
				spawnRainParticle();
			}
		}

		// Update all active rain particles
		for (let i = 0; i < rainParticles.length; i++) {
			...
			// Apply gravity for realistic downward acceleration
			...
			// Check if particle hits the ground and create splash effect
			if (particle.position.y <= 0 && !particle.hasSplashed) {
				// Create splash effect using ring mesh
				...
				// Mark this particle as having splashed to avoid duplicate effects
				particle.hasSplashed = true;
			}

			// Despawn particles that fall below ground level
...
// Control visibility of the grab mesh based on configuration
