...
const action = app.create('action', {
  ...
  onTrigger: (eventData) => {
    ...
    if (eventData && eventData.playerId) {
      ...
      if (player) {
        playerName = (player.name && player.name !== 'Anonymous') ? player.name : playerId;
      } else {
        playerName = playerId; 
      }
      console.log(`[rocket.js] Rocket triggered by ${playerName} (ID: ${playerId}).`);

      // Start launch sequence
      action.launching = true;
      action.launchStartTime = world.getTime();
      action.active = false; 
      const announcementMessage = `${playerName} launched the rocket! 🚀`;
      console.log(`[rocket.js] Sending rocket launch announcement to in-game chat: "${announcementMessage}"`);
      const chatPayload = {
      ...
      world.chat(chatPayload, true); 

    } else {
      // Handle case where eventData or eventData.playerId is missing
      // Check if it's an object without playerId, or something else
      let receivedType = typeof eventData;
      if (eventData && typeof eventData === 'object' && !eventData.playerId) {
        console.log('[rocket.js] Rocket action onTrigger triggered with eventData object, but missing playerId property.');
      } else if (!eventData) {
        console.log('[rocket.js] Rocket action onTrigger triggered without eventData.');
      } else {
        console.log(`[rocket.js] Rocket action onTrigger triggered with unexpected data type for eventData: ${receivedType}. Value: ${JSON.stringify(eventData)}`);
      }
      console.log('[rocket.js] Cannot launch or announce rocket.');
      if (typeof particles !== 'undefined' && particles && typeof particles.rate === 'number') {
        particles.rate = 0; 
      }
      return; 
    }
  ...
  onCancel: (eventData) => {
    ...
    if (eventData && eventData.playerId) {
      ...
      console.log(`Launch sequence cancelled by ${playerName}`);
    } else {
      console.log('Rocket action onCancel triggered (no specific player identified).');
    }
