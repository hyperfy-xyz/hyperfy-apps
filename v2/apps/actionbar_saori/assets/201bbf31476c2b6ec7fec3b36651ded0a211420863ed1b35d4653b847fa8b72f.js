if (world.isClient) {
  ...
  // Define a unique icon URL for each slot
  const slotIcons = [
    ...
    'https://wow.zamimg.com/images/wow/icons/large/spell_shadow_shadowfury.jpg', // Changed V key icon
  ...
  for (let i = 0; i < 10; i++) {
    ...
    slot.onPointerDown = () => {
      ...
      console.log(`Activated ability in slot ${i+1} (${keybinds[i]} key)`);
    };
  ...
  for (let i = 0; i < keybinds.length; i++) {
    ...
    controls[`key${key}`].onRelease = () => {
      slot.backgroundColor = CONFIG.slot.backgroundColor;
      // execute action here based on key
      switch(key) {
        case 'Q':
          const matrix = world.getPlayer().getBoneTransform('chest')
          const bonePosition = new Vector3().setFromMatrixPosition(matrix).toArray() 
...
if (world.isServer) {
  app.on('keyQ', ({playerId, bonePosition}) => {
    console.log('boneposition', bonePosition)
    const {quaternion} = world.getPlayer(playerId)
    const forward = new Vector3(0, 0, -1)
    const direction = new Vector3().copy(forward).applyQuaternion(quaternion)
    app.emit('raybeam', {
