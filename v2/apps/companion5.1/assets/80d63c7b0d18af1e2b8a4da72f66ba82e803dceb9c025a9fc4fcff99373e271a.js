        ...
        case 'guiding':
            if (objectState.guidance.targetObject && objectState.guidance.targetObject.position) {
                ...
                if (distanceToTarget < MIN_FOLLOW_DISTANCE) {
                    updateAnimation(ANIMATIONS.IDLE);
                    // Calculate position to stop at, just like in following case
                    const scale = MIN_FOLLOW_DISTANCE / distanceToTarget;
                    targetPosition = {
...
function moveToTarget(target, delta) {
    const distance = getDistance(app.position, target);
    
    // If we're following and within minimum distance, stop moving
    if (objectState.currentState === 'following' && distance <= MIN_FOLLOW_DISTANCE) {
        // When stopped, face directly at the target
        const dirToTarget = {
            x: target.x - app.position.x,
            z: target.z - app.position.z
        };
        // For following, we want to face TOWARDS the target
        const faceAngle = Math.atan2(dirToTarget.x, dirToTarget.z) + Math.PI;
        let currentAngle = app.rotation.y;
        let angleDiff = faceAngle - currentAngle;
        while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
    ...
    direction.x = target.x - app.position.x;
    direction.z = target.z - app.position.z;
    
    app.position.x += direction.x * ratio;
    app.position.z += direction.z * ratio;
    app.position.y = 11.2;
    
    // For movement, we want to face the direction we're moving
