...
function createCameraPanel() {
  ...
  freeGroup.add(createStepperControl('Movement Easing', () => CONFIG.FREECAM_MOVE_EASING, v => { CONFIG.FREECAM_MOVE_EASING = v }, 0, 1, 0.01, 0.1))
  ...
  freeGroup.add(createStepperControl('Look Easing', () => CONFIG.FREECAM_LOOK_EASING, v => { CONFIG.FREECAM_LOOK_EASING = v }, 0.01, 1, 0.01, 0.1))
  ...
  orbitGroup.add(createStepperControl('Movement Easing', () => CONFIG.ORBIT_MOVE_EASING, v => { CONFIG.ORBIT_MOVE_EASING = v }, 0, 1, 0.01, 0.1))
  ...
  orbitGroup.add(createStepperControl('Look Easing', () => CONFIG.ORBIT_LOOK_EASING, v => { CONFIG.ORBIT_LOOK_EASING = v }, 0.02, 1, 0.01, 0.1))
...
// Map a 0-1 ease value to a per-frame lerp/slerp alpha taking frame time into account.
function easeAlpha(easeValue, delta) {
  // Original linear easing for orbit cam
  const alpha = (1 - easeValue) * delta * 60
  return Math.min(1, Math.max(0.0001, alpha))
}

// Map a 0-1 ease value with power curve for better freecam sensitivity distribution
function freecamEaseAlpha(easeValue, delta) {
  // Apply inverse power curve - use square root to spread sensitivity across range
  const curvedEase = Math.sqrt(easeValue)
  // easeValue 0 => instant movement (alpha 1), 1 => maximum smoothing (alpha ~0)
  const alpha = (1 - curvedEase) * delta * 60
  return Math.min(1, Math.max(0.0001, alpha))
}
...
function updateOrbit(delta) {
  ...
  // Smooth movement using configurable easing
  cameraPosition.lerp(cameraTarget, easeAlpha(CONFIG.ORBIT_MOVE_EASING, delta))
  control.camera.position.copy(cameraPosition)

  lookAtTarget(targetPos, easeAlpha(CONFIG.ORBIT_LOOK_EASING, delta))
}
...
function updateFreecam(delta) {
  ...
  freecamVelocity.lerp(desiredVelocity.applyQuaternion(control.camera.quaternion), 0.05)
  ...
  if (freecamTargetQuat) control.camera.quaternion.slerp(freecamTargetQuat, 0.05)
}
