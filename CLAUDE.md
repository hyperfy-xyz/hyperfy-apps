...
uv run python scripts/hyp_tool.py info v2/hyp-files/SomeApp.hyp
uv run python scripts/hyp_tool.py unbundle v2/hyp-files/SomeApp.hyp output_dir/
```
...
```
v2/hyp-files/*.hyp (binary)
    → scripts/hyp_tool.py unbundle
    → v2/apps/<slug>/ (blueprint JSON + index.js + assets/)
    → scripts/catalog/build_catalog.py
    → scripts/context/apps/<slug>/manifest.json  (manifest["ai"] merged from tmp/ai-summaries/)
...
GitHub Pages auto-deploys `catalog/` on push to `main` when `catalog/**` changes (`.github/workflows/deploy-pages.yml`). The explorer uses relative paths — preview images resolve under `catalog/media/<slug>/`, downloads point to GitHub raw CDN for `v2/hyp-files/*.hyp`.
